...
class IsOwnerOrAdminForUserMediaFiles(permissions.BasePermission):
    """
    Custom permission for UserMediaFiles:
      - SAFE_METHODS: Allowed for any authenticated user.
      - Create/Update: Allowed only if the requesting user is the owner (i.e. UserMediaFiles.user).
      - Delete: Allowed only for superuser or staff.
    """
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # DELETE is allowed only for superuser/staff.
        if request.method == 'DELETE':
            return request.user.is_superuser or request.user.is_staff

        # For POST, PUT, PATCH: allow if the file's owner is the same as the requesting user.
